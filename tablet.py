...
# ---------------- Config (Fire 2) ----------------
INPUT_XLSX  = "Fire 2 Data.xlsx"
OUTPUT_XLSX = "Fire 2 Data Proceed.xlsx"
ID_COL = 0  # first column in Excel (0-based)

# NEW: base directory to search CSVs in (non-recursive)
SEARCH_DIR = "../Data/fire2data"

# Write destinations (0-based, inclusive ranges) — 11 columns each
RANGES = {
    "evolab5e+tablet":       (54, 64),  # Excel cols 55..65
    "intermediate+tablet":   (65, 75),  # Excel cols 66..76
    "standardoffice+tablet": (76, 86),  # Excel cols 77..87
}
...
KEYS = {
    ...
    "standardoffice+tablet": ("standard office", "tablet"),
}
...
def main():
    ...
    # Ensure columns exist up to the highest needed index (86)
    max_col_needed = max(r[1] for r in RANGES.values())
    df = ensure_columns(df, max_col_needed)

    # Pre-cast all destination blocks to 'object' once (avoids dtype warnings)
    all_block_cols = list(range(54, 65)) + list(range(65, 76)) + list(range(76, 87))
