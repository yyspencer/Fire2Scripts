...
S1_COL = 8  # Sheet 1 (overall) → column 9
S2_COL = 3  # Sheet 2 (pre)     → column 4
S3_COL = 3  # Sheet 3 (post)    → column 4
...
def idx5_direct(v) -> str:
    """
    Fire 2 rule: direct index (no trimming).
...
def count_robotface_looks(rows, look_col):
    """
    Count contiguous runs where LookingAt == 'smoothfaced' (exact match).
...
def main():
    ...
    try: df_s1.columns.values[S1_COL] = "Robotface Look Count (Overall)"
    except Exception: pass
    try: df_s2.columns.values[S2_COL] = "Robotface Look Count (Pre)"
    except Exception: pass
    try: df_s3.columns.values[S3_COL] = "Robotface Look Count (Post)"
    except Exception: pass
    ...
    for i in range(len(df_s1)):
        ...
        try:
            with open(csv_path, newline="", encoding="utf-8") as f:
                ...
                # Overall robotface look count
                overall_looks = count_robotface_looks(rows, look_col)
                ...
                pre_looks  = count_robotface_looks(pre_rows,  look_col)
                post_looks = count_robotface_looks(post_rows, look_col)
    ...
    print("\nDone! Robotface Look Count written to: Sheet1 col 9 (overall), Sheet2/3 col 4 (pre/post).")
