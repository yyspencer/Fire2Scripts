...
def analyze_index(idx5: str):
    """
    Returns (folder_type, delta_seconds, (abs_x, abs_y, abs_z)) or (None, None, None) if skipped.
    ...
    # crisis row (nearest in time)
    i_crisis = nearest_time_index(t, crisis_time)

    # crisis position (original values for dwell logic)
    ...
    # find start/end of dwell window at EXACT (cx,cy,cz)
    i_start = i_crisis
    ...
    # The requested metric: time difference from arrival at that location to crisis time
    delta_sec = crisis_time - start_time if (np.isfinite(crisis_time) and np.isfinite(start_time)) else np.nan

    # Report ABSOLUTE coordinates for the crisis location
    acx, acy, acz = abs(cx), abs(cy), abs(cz)
    pos_str = f"({acx}, {acy}, {acz})"

    if np.isfinite(delta_sec):
        print(f"{idx5} [{folder_type}]: crisis_time={crisis_time:.6f}s, pos={pos_str}, "
              f"start={start_time:.6f}s, end={end_time:.6f}s, delta={delta_sec:.6f}s")
    else:
        print(f"{idx5} [{folder_type}]: crisis_time={crisis_time:.6f}s, pos={pos_str}, "
              f"start={start_time:.6f}s, end={end_time:.6f}s, delta=NaN")

    return folder_type, (delta_sec if np.isfinite(delta_sec) else None), (acx, acy, acz)
...
def main():
    ...
    shook_delta, nosh_delta = [], []
    shook_x, shook_y, shook_z = [], [], []
    nosh_x, nosh_y, nosh_z    = [], [], []

    for idx5 in indices:
        ftype, delta_sec, pos = analyze_index(idx5)
        if ftype is None:
            continue
        if delta_sec is not None:
            if ftype == "shook":
                shook_delta.append(delta_sec)
            elif ftype == "noshook":
                nosh_delta.append(delta_sec)
        if pos is not None:
    ...
    def diff_stats(label, arr):
        if arr:
            a = np.array(arr, dtype=float)
            mean = float(np.nanmean(a))
            var  = float(np.nanvar(a, ddof=1)) if len(a) > 1 else float('nan')
            print(f"{label} delta (sec): count={len(arr)}, mean={mean:.6f}, sample variance={var:.6f}")
        else:
            print(f"{label} delta (sec): count=0 (no valid values)")

    diff_stats("shook", shook_delta)
    diff_stats("noshook", nosh_delta)
    ...
    def loc_stats(label, xs, ys, zs):
        if xs and ys and zs:
            ...
            print(f"{label} crisis location mean (abs) : ({mx:.6f}, {my:.6f}, {mz:.6f})")
            print(f"{label} crisis location var  (abs) : ({vx:.6f}, {vy:.6f}, {vz:.6f})")
        else:
