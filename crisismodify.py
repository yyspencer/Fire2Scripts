...
# ========= Config =========
EXCEL_FILE = "Fire 2 Data.xlsx"   # first sheet
ID_COL = 0                        # indices column (0-based)
CRISIS_COL = 3                    # "Crisis Time" lives in the 3rd col (0-based)
CSV_FOLDER = "noshookmodified"    # only search here (no baseline folders)

# Learned locations (ABS positions), means & variances (per-axis), and radii = 2σ
ENTER_MEAN = np.array([24.492535, 0.0, 45.259259], dtype=float)
ENTER_VAR  = np.array([ 0.001239, 0.0,  0.000274], dtype=float)

EXIT_MEAN  = np.array([33.641574, 0.0, 45.199716], dtype=float)
EXIT_VAR   = np.array([ 0.000803, 0.0,  0.001289], dtype=float)
...
def main():
    ...
    print(f"Will process {len(todo_rows)} rows with blank Crisis Time, using CSVs in '{CSV_FOLDER}/'.")

    for i in todo_rows:
        raw_id = df_idx.iat[i, ID_COL]
        idx5 = index5(raw_id)

        # find CSV in noshookmodified
        csv_path = None
        if os.path.isdir(CSV_FOLDER):
            for name in os.listdir(CSV_FOLDER):
                if name.lower().endswith(".csv") and name[:5] == idx5:
                    ...
                    break
        if not csv_path:
            print(f"{idx5}: SKIP (no CSV found in '{CSV_FOLDER}/').")
            continue
        ...
        # --- New: choose the analysis window (proximity → roomEvent tags → full range)
        ...
        if s_idx is None or e_idx is None or e_idx < s_idx:
            print(f"{idx5}: SKIP (no stationary session inside crisis sphere in {win_src} window).")
            continue

        # time robot starts moving after longest stationary session
        if e_idx + 1 >= n or not np.isfinite(t[e_idx + 1]):
            print(f"{idx5}: SKIP (cannot find movement time after stationary session).")
            continue
        ...
        if not np.isfinite(t[j]) or abs(float(t[j]) - target_time) > DELTA_WIN:
            print(f"{idx5}: SKIP (no row within ±2σ of predicted crisis time; "
                  f"nearest diff={abs(float(t[j])-target_time):.6f}s).")
            continue
        ...
        try:
            ...
            print(f"{idx5}: OK [{win_src}] — {ent_info}, {ex_info}, "
                  f"move@{t[e_idx+1]:.3f}s, estimated_shook@{t[j]:.3f}s (Δ={t[j]-move_time:.3f}s).")
        except Exception as e:
            print(f"{idx5}: ❌ Failed to write CSV: {e}")
